...
export function useVariables() {
    ...
    const variablesQuery = useQuery({
        ...
        queryFn: async (): Promise<{
            ...
            try {
                const variablesData = await apiFetch<{
                    data?: StaticVariableApiResponse[];
                }>("/variables");
                return {
    ...
    const addVariableMutation = useMutation({
        mutationFn: async (variable: Omit<Variable, "id">) => {
            if (!token)
                throw new Error(
                    "Autenticação necessária para criar variáveis.",
                );
            const data = await apiFetch<StaticVariableApiResponse>(
                "/variables",
                {
                    method: "POST",
                    token,
    ...
    const updateVariableMutation = useMutation({
        mutationFn: async ({
            ...
            if (!token)
                throw new Error(
                    "Autenticação necessária para editar variáveis.",
                );
            const data = await apiFetch<StaticVariableApiResponse>(
                `/variables/${id}`,
                {
    ...
    const deleteVariableMutation = useMutation({
        mutationFn: async (id: string) => {
            if (!token)
                throw new Error(
                    "Autenticação necessária para excluir variáveis.",
                );
