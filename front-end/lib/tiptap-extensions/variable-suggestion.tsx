...
>(function VariableList({ items, command }, ref) {
    ...
    return (
        <div
            ref={innerRef}
            className="bg-popover border border-border rounded-lg shadow-lg overflow-hidden min-w-60 max-h-75 overflow-y-auto"
        >
...
function createSuggestionConfig(variableItemsRef: {
    current: Array<{ variable_name: string; description: string }>;
}) {
    return {
        char: "{{",
        allowSpaces: true,
...
export function createVariableSuggestionExtension(variableItemsRef: {
    current: Array<{ variable_name: string; description: string }>;
}) {
