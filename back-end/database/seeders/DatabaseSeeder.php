...
class DatabaseSeeder extends Seeder
{
    public function run()
    {
        ...
        $user = User::query()
            ->where("email", "admin@instituicao.com")
            ->firstOrFail();
        ...
        if (file_exists($dumpPath) && app()->environment() !== "testing") {
            $this->command->info(
                "Iniciando importação automática do dump.sql via ETL...",
            );
        ...
        foreach ($this->addresses() as $addressData) {
            DB::table("addresses")->updateOrInsert(
                ["cep" => $addressData["cep"]],
                [...$addressData, "updated_at" => now(), "created_at" => now()],
            );
        }
